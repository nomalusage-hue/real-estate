...
export default function PropertyFilters({ filters, onChange, stats }: PropertyFiltersProps) {
  ...
  // Set default status on first load if not already set
  useEffect(() => {
    if (!initialDefaultSet.current && (!filters.status || filters.status.length === 0)) {
      initialDefaultSet.current = true;
      onChange({ ...filters, status: ["For Sale", "For Rent"] });
    }
  }, [filters, onChange]);
  ...
  // Reset all filters, but keep default status
  const handleReset = () => {
    setSearchTerm('');
    onChange({ status: ["For Sale", "For Rent"] });
  };
